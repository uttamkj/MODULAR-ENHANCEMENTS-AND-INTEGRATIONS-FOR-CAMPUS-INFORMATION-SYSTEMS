...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($course === "All") {
        $sql = "SELECT status FROM voting_status WHERE vote_type = ? AND course = 'All'";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("s", $voteType);
    } else {
        // Handle branch-specific and common vote types
        $sql = "SELECT status FROM voting_status WHERE vote_type = ? AND (course = ? OR course = 'All')";
        $stmt = $conn->prepare($sql);
    ...
    if ($row) {
        ...
        // Update the status
        if ($course === "All") {
            $updateSQL = "UPDATE voting_status SET status = ? WHERE vote_type = ? AND course = 'All'";
            $updateStmt = $conn->prepare($updateSQL);
            $updateStmt->bind_param("ss", $newStatus, $voteType);
        } else {
            $updateSQL = "UPDATE voting_status SET status = ? WHERE vote_type = ? AND (course = ? OR course = 'All')";
            $updateStmt = $conn->prepare($updateSQL);
        ...
        if ($updateStmt->execute()) {
            ...
            echo json_encode(["status" => "error", "message" => "Failed to update voting status"]);
            exit;
        }
    } else {
